...
export function matches(m: any): boolean {
  ...
  if (lower(m.compat?.thinkingFormat) === "longcat") return true;
  const p = lower(m.provider);
  if (p === "longcat" || p === "longcat-coding") return true;
  const h = baseHost(m.baseUrl || m.base_url);
  return h === "api.longcat.chat" || h.endsWith(".longcat.chat");
}
...
function should(p: any, m: any, o: any): boolean {
...
export function apply(p: any, m: any, o: any = {}): any {
  ...
  return should(p, m, o) ? disable(p) : p;
}
